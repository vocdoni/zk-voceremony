...
check_create_env() {
	...
	echo " - The global artifacts and initial contribution will be stored in '$CONTRIBUTIONS_PATH' folder."
...
compile_circuit() {
	...
	mv $CONTRIBUTIONS_PATH/${CIRCUIT_FILENAME}_js/$CIRCUIT_FILENAME.wasm $CONTRIBUTIONS_PATH/$CIRCUIT_FILENAME.wasm
	# remove the js folder
	rm -rf $CONTRIBUTIONS_PATH/${CIRCUIT_FILENAME}_js
}

prepare_contribution() {
	log "prepare the contribution using '$INPUT_PTAU_PATH' as initial ptau file"
	$SNARKJS groth16 setup $CONTRIBUTIONS_PATH/$CIRCUIT_FILENAME.r1cs $INPUT_PTAU_PATH $CONTRIBUTIONS_PATH/${CIRCUIT_FILENAME}_initial_contribution.zkey
}

save_initial_contribution() {
	log "save the initial contribution to '$CONTRIBUTIONS_FILE'"
	# calculate the hashes of the global artifacts and initial contribution
	local r1cs_hash=$(get_file_hash "$CONTRIBUTIONS_PATH/$CIRCUIT_FILENAME.r1cs")
	local wasm_hash=$(get_file_hash "$CONTRIBUTIONS_PATH/$CIRCUIT_FILENAME.wasm")
	local initial_ptau_hash=$(get_file_hash "$INPUT_PTAU_PATH")
	local initial_zkey_hash=$(get_file_hash "$CONTRIBUTIONS_PATH/${CIRCUIT_FILENAME}_initial_contribution.zkey")
...
\`\`\`"
	...
	echo "$contributions_content" > $CONTRIBUTIONS_FILE
}
